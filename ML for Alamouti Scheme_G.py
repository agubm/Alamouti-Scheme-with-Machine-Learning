...
noOfSamples = int(144000) #number of observations/sample data generated in the for-loop. It determines the 3 dimension of our matrix or vector.
...
def Alamouti_Scheme_A():
    ...
    model.add(Dense(5, init = 'random_uniform', activation='sigmoid'))#Hidden layer,
...
def Alamouti_Scheme_C():
    ...
    model.add(Dense(encoding_dim, activation='relu', activity_regularizer=regularizers.l1(1e-5)))#Hidden layer,
    model.add(Dense(3 , init = 'uniform', activation='reul'))# Hidden layer
...
def Alamouti_Scheme_D():
    ...
    layerC = Dense(intermediate_dim, activation='relu')(layerB)
    
    #defines the latent layers
    z_mu = Dense(latent_dim, name = 'latent_mu')(layerC) #mean of the encoded sample
    z_sigma = Dense(latent_dim, name = 'latent_sigma')(layerC) #standard deviation of the encoded samples
    
    #to  specify a joint distribution over the observed and latent variables via a somewhat inference network:
    def sample_z(args):
        z_mu, z_sigma = args
    ...
    #sample the latent space using the KL Reparameterization trick
    z = Lambda(sample_z, output_shape = (latent_dim), name = 'latent_layer')([z_mu, z_sigma])
     
    #intantiate encoder:
    encoder = Model(layerA, [z_mu, z_sigma, z], name = 'VAE Encoder')
    encoder.summary()
    
    
    #define the input (decoder) layers:
    layerD = Input(shape=(latent_dim, ), name='decoder_input')
    layerE = Dense(intermediate_dim, activation='relu')(layerD)
    layerF = Dense(original_dim_b, activation='relu')(layerE)
    layerG = Dense(original_dim, activation='sigmoid')(layerF)
   
    #intantiate decoder:
    decoder = Model(layerD, layerG, name = 'VAE Decoder')
    decoder.summary()
    
    #intantiate the Variation AutoEncoder
    # Instantiate VAE
    vae_outputs = decoder(encoder(layerA)[2])
    vae = Model(layerA, vae_outputs, name='vae')
    vae.summary()
    ...
    vae = Model(inputs=[layerA, eps], outputs=x_pred)
    #vae.compile(optimizer='adam', loss=nll)
    vae.compile(optimizer = 'adam', loss = 'mse',  metrics = ['acc'])
    
    #autoencoder will learn the distribution of sample ('symbols') across (two)dimensional latent space
