...
#to be able to reeuse the content of the variable: 
...
def Alamouti_Scheme_A():
    ...
    #train the model now:
    AutoEncoder1 = model.fit(Symobl_est_all_training, Symobl_all_training, validation_data = (Symobl_est_all_validation, Symobl_all_validation), epochs=epoch, batch_size =  dim, verbose= 1)
...
def Alamouti_Scheme_B():
    ...
    autoencoder.fit(Symobl_est_all_training_B, Symobl_all_training_B, epochs= epoch, batch_size=dim, shuffle=True, validation_data=(Symobl_est_all_validation_B, Symobl_all_validation_B))
...
def Alamouti_Scheme_C():
    ...
    #early_stopping  = tf.keras.callbacks.EarlyStopping(monitor = 'val_loss', patience = 10, mode = 'min') # here, the model stop during training it the validation loss does not improves after 2 consecutive runs
     
    autoencoder_withSparsity.compile(optimizer = 'adam', loss = 'mse')
    autoencoder_withSparsity.fit(Symobl_est_all_training_C, Symobl_all_training_C, epochs= epoch, batch_size=dim, shuffle=True, validation_data=(Symobl_est_all_validation_C, Symobl_all_validation_C))
...
x_train =  Symobl_all_training_D
x_test = Symobl_all_validation_D
